...
export default {
  ...
  methods: {
    ...
    async createProduct() {
      try {
        await axios.post('http://localhost:8081/api/products', this.newProduct);
    ...
    async updateProduct() {
      try {
        await axios.put(`http://localhost:8081/api/products/${this.selectedProduct.id}`, this.selectedProduct);
        this.fetchProducts(); // Recargar lista de productos
